...
def _scrape_url(url: str) -> str:
    """Fetch and extract plain text from a URL using httpx + BeautifulSoup."""
    try:
...
def _parse_file(filename: str, content_bytes: bytes) -> str:
    """Parse uploaded file bytes into plain text based on extension."""
...
async def upload_document(
    ...
    """Parse and store a knowledge document (PDF, DOCX, TXT, CSV, XLSX/XLS)."""
    # ── Extension check ──
...
async def add_url_document(
    ...
    """Scrape a URL and store its text content as a knowledge document."""
    ...
    # ── Check if this URL already exists (update in place instead of duplicate) ──
...
# ─────────────────────────────────────────────────────────────────────────────
# POST /admin/refresh-urls  (called internally by the scheduler)
# ─────────────────────────────────────────────────────────────────────────────
@router.post("/refresh-urls")
async def refresh_url_documents(tenant_id: str = Query(...), db: Session = Depends(get_tenant_db)):
    """Re-scrape all active URL-type knowledge documents for this tenant."""
...
async def get_documents(db: Session = Depends(get_tenant_db)):
    """List all active knowledge documents for this tenant."""
